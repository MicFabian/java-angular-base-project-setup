...
2. Keep production code under `server/src/main/java/com/example/baseproject/api` and organize it with the repository roots `controller`, `domain`, `accessor`, and `shared`, using feature names below `controller`, `domain`, and `accessor`.
3. Prefer constructor injection and explicit request/response types for HTTP endpoints.
4. Use domain-owned accessor interfaces for infrastructure communication: domain use cases depend on the accessor abstraction; `accessor/<feature>` provides the implementation.
